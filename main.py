...
EXCLUDED_KEYS = {Key.f1,keyboard.Key.f2,keyboard.Key.f3,keyboard.Key.f4,keyboard.Key.f5,keyboard.Key.f6,keyboard.Key.f7,keyboard.Key.f8,keyboard.Key.f9,keyboard.Key.f10,keyboard.Key.f11,keyboard.Key.f12}
...
def copyTextAndSavePage(message):

    # send_keys("^c")

    fixed_message = fixClipboardText()
...
def evaluatePressedKeys(key,pressed):
    ...
    if pressed and not strKey in pressedKeys:
        ...
        if strKey in CTRL_SHIFT_S:
            if all(k in pressedKeys for k in CTRL_SHIFT_S):
                ...
                copyTextAndSavePage(getClipboardText())
...
def on_press_test(key):
    ...
    try: logger.debug('CHAR', key.char) # letters, numbers et
    except: logger.debug('NAME', keyboard.KeyCode().from_char(key.name))

def on_press(key):
    evaluatePressedKeys(key,True)

def on_release(key):
    return evaluatePressedKeys(key,False)
